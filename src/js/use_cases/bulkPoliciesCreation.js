...
const bulkPolicyInstanceCreationCompletionTrend = Trend('PolicyInstanceCreationCompletionBulkDuration', true);

export default function (PARAMSCCF) {
    group('A1 Policy Instance Bulk Operations', function () {
        group('CREATE multiple policy instances', function () {
            sleep(5);
            ...
            console.log("policies are getting created bulk(status): ", putBulkPolicy.status);
            console.log("policies are getting created bulk(body): ", putBulkPolicy.body);
            let duration = putBulkPolicy.timings.duration;
            bulkPolicyInstanceCreationCompletionTrend.add(duration);
            const putBulkPolicyResponse = check(putBulkPolicy, {
                'Policies are getting created': (r) => r.status === 201,
            });
        });
